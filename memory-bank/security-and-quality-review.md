...
Revisión del código — **2026-06-04**. Proyecto demo con datos mock; riesgos críticos aplican si se despliega más allá de desarrollo local.
...
| 🟡  | **npm audit: brace-expansion** (moderate, transitive)                  | `frontend/node_modules`                    | `npm audit fix`                                   |
| 🟡  | **npm audit: postcss** (moderate, direct)                              | `frontend/node_modules/postcss`            | Actualizar a `postcss >= 8.5.10`                  |
...
- Accesibilidad UI: skip link, `aria-busy`, `role="status"`, iconos decorativos con `aria-hidden`, y `focus-visible` global.
- Rendimiento frontend: code splitting de charts con `React.lazy` + `Suspense`, y optimización de `computeKPIs` a un solo recorrido.
- Higiene documental: referencias obsoletas eliminadas/actualizadas en skills locales.
...
2. Validación Zod en `financial-api.ts`
3. Error Boundary + botón retry
4. Ejecutar `npm audit fix` y revisar impacto en lockfile
